...
class LangFiles
{
    ...
    public function displayInputs($fileArray, $parents = [], $parent = '', $level = 0)
    {
        ...
        foreach ($fileArray as $key => $item) {
            if (is_array($item)) {
                echo view()->make('langs.language_headers', ['header' => $key, 'parents' => $parents, 'level' => $level, 'item' => $item, 'langfile' => $this, 'lang_file_name' => $this->file])->render();
            } else {
                echo view()->make('langs.language_inputs', ['key' => $key, 'item' => $item, 'parents' => $parents, 'lang_file_name' => $this->file])->render();
            }
        }
    }
